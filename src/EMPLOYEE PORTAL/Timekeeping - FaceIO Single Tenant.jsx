...
const Timekeeping = () => {
  ...
  const getFaceIOErrorMessage = useCallback((error) => {
  ...
  const authenticateWithFaceIO = useCallback(async () => {
    ...
    try {
      const result = await faceioRef.current.authenticate({ locale: "auto" });
  ...
  }, [isImageCaptureRequired, resetFaceIO]);
  ...
  }, [isImageCaptureRequired, initializeFaceIO]);
